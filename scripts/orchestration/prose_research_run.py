...
def main() -> int:
    ...
    if args.build_report and not args.dry_run:
        planner_eval_path = run_dir / "artifacts" / f"planner_family_eval.{tag}.json"
        ...
        planner_eval = load_json(str(planner_eval_path)) if planner_eval_path.exists() else {}
        selected_direct_family_id = compact_ws(planner_eval.get("selected_direct_family_id"))
        selected_related_family_id = compact_ws(planner_eval.get("selected_related_family_id"))

        direct_evidence_for_report = None
        related_evidence_for_report = None

        if planner_eval_path.exists() and selected_direct_family_id:
            direct_tag = f"direct_{selected_direct_family_id}"
            ...
            direct_cmd = [
                ...
                "--orchestration-plan", plan_path,
                "--artifact-tag", direct_tag,
                "--write", str(direct_materialize_out),
            ]
            print()
            print("## prose_materialize_family (direct)")
            run_cmd(direct_cmd, dry_run=False)
            summary["actions"].append("prose_materialize_family_direct")
            summary["artifacts"]["direct_family_materialize"] = str(direct_materialize_out)

            direct_candidate = run_dir / "artifacts" / f"planner_selected_evidence.{direct_tag}.json"
            if direct_candidate.exists():
                direct_evidence_for_report = direct_candidate

        if planner_eval_path.exists() and selected_related_family_id:
            related_tag = f"related_{selected_related_family_id}"
            ...
            related_cmd = [
                ...
                "--orchestration-plan", plan_path,
                "--artifact-tag", related_tag,
                "--write", str(related_materialize_out),
            ]
            print()
            print("## prose_materialize_family (related)")
            run_cmd(related_cmd, dry_run=False)
            summary["actions"].append("prose_materialize_family_related")
            summary["artifacts"]["related_family_materialize"] = str(related_materialize_out)

            related_candidate = run_dir / "artifacts" / f"planner_selected_evidence.{related_tag}.json"
            if related_candidate.exists():
                related_evidence_for_report = related_candidate

        portfolio_report_input_out = run_dir / "artifacts" / f"prose_portfolio_report_input.{tag}.json"
        standard_report_input_out = run_dir / "artifacts" / f"prose_run_report_input.{tag}.json"

        report_input_out = standard_report_input_out
        ...
        if direct_evidence_for_report and related_evidence_for_report:
            portfolio_cmd = [
                ...
                "--related-evidence-input", str(related_evidence_for_report),
                "--orchestration-plan", plan_path,
                "--write", str(portfolio_report_input_out),
            ]
            print()
            print("## prose_portfolio_report_input")
            run_cmd(portfolio_cmd, dry_run=False)
            summary["actions"].append("prose_portfolio_report_input")
            summary["artifacts"]["prose_portfolio_report_input"] = str(portfolio_report_input_out)

            portfolio_payload = load_json(str(portfolio_report_input_out))
            direct_count = len(portfolio_payload.get("direct_evidence_articles") or [])
            related_count = len(portfolio_payload.get("related_evidence_articles") or [])

            if direct_count > 0 or related_count > 0:
                report_input_out = portfolio_report_input_out
                portfolio_mode = True

                direct_evidence_for_router = direct_evidence_for_report
                related_evidence_for_router = related_evidence_for_report

                if direct_evidence_for_router and related_evidence_for_router:
                    ...
                    advisory_cmd = [
                        ...
                        "--related-evidence-input", str(related_evidence_for_router),
                        "--portfolio-input", str(portfolio_report_input_out),
                        "--orchestration-plan", plan_path,
                        "--memory-path", args.memory_path,
                        "--write", str(advisory_summary_out),
                    ]
                    print()
                    print("## prose_evidence_router_advisory")
                    run_cmd(advisory_cmd, dry_run=False)
                    summary["actions"].append("prose_evidence_router_advisory")
                    summary["artifacts"]["evidence_router_advisory"] = str(advisory_summary_out)

                    advisory_summary = load_json(str(advisory_summary_out))
                    advisory_input_path = advisory_summary.get("advisory_input")
                    if advisory_summary.get("use_advisory_input") and advisory_input_path:
                        report_input_out = Path(advisory_input_path)
        ...
        if not portfolio_mode:
            report_input_cmd = [
                ...
                "--write", str(standard_report_input_out),
            ]
            print()
            print("## prose_run_report_input")
            run_cmd(report_input_cmd, dry_run=False)
            summary["actions"].append("prose_run_report_input")
            summary["artifacts"]["prose_run_report_input"] = str(standard_report_input_out)
        ...
        print()
        print("## prose_report_input_enrich")
        run_cmd(enrich_report_input_cmd, dry_run=False)
        summary["actions"].append("prose_report_input_enrich")
        summary["artifacts"]["prose_report_input_enriched"] = str(enriched_report_input_out)
        report_input_out = enriched_report_input_out
